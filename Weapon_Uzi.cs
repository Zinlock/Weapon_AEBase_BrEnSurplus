datablock AudioProfile(BNE_UziFire1Sound)
{
   filename    = "./Sounds/Fire/Uzi/Uzi_fire1.wav";
   description = LightClose3D;
   preload = true;
};

datablock AudioProfile(BNE_UziFire2Sound)
{
   filename    = "./Sounds/Fire/Uzi/Uzi_fire2.wav";
   description = LightClose3D;
   preload = true;
};

datablock AudioProfile(BNE_UziFire3Sound)
{
   filename    = "./Sounds/Fire/Uzi/Uzi_fire3.wav";
   description = LightClose3D;
   preload = true;
};

// Uzi
datablock DebrisData(BNE_UziMagDebris)
{
	shapeFile = "./Uzi/UziMag.dts";
	lifetime = 2.0;
	minSpinSpeed = -200.0;
	maxSpinSpeed = -100.0;
	elasticity = 0.5;
...
// item //
//////////
datablock ItemData(BNE_UziItem)
{
	...
	recoil = "Medium";
	uiColor = "1 1 1";
	description = "The Uzi is a compact submachine gun designed by Uziel Gal in 1950 which is still in use today.";
...
//weapon image//
////////////////
datablock ShapeBaseImageData(BNE_UziImage)
{
   // Basic Item properties
   shapeFile = "./Uzi/Uzi.dts";
   emap = true;
   ...
   //raise your arm up or not
	armReady = true;
	hideHands = false;
	safetyImage = BNE_UziSafetyImage;
    scopingImage = BNE_UziIronsightImage;
	doColorShift = true;
	colorShiftColor = BNE_UziItem.colorShiftColor;//"0.400 0.196 0 1.000";
...
{	
	%obj.stopAudio(0); 
  %obj.playAudio(0, BNE_UziFire @ getRandom(1, 3) @ Sound);
...
function BNE_UziImage::onDryFire(%this, %obj, %slot)
{
	%obj.aeplayThread(2, plant);
	serverPlay3D(AEDryFireSound, %obj.getHackPosition());
}

function BNE_UziImage::onReloadMagIn(%this,%obj,%slot)
{
   %obj.schedule(50, "aeplayThread", "2", "shiftright");
}

function BNE_UziImage::onReloadMagOut(%this,%obj,%slot)
{
   %obj.aeplayThread(2, wrench);
   %obj.aeplayThread(3, plant);
}

function BNE_UziImage::onReloadEnd(%this,%obj,%slot)
{
	Parent::AEMagReloadAll(%this, %obj, %slot);
}

function BNE_UziImage::onReload2End(%this,%obj,%slot)
{
	Parent::AEMagReloadAll(%this, %obj, %slot);
}
...
function BNE_UziImage::onReloadStart(%this,%obj,%slot)
{
...
function BNE_UziImage::onMount(%this,%obj,%slot)
{
	%obj.aeplayThread(2, plant);
...
function BNE_UziImage::onUnMount(%this,%obj,%slot)
{
...
function BNE_UziImage::onMagDrop(%this,%obj,%slot)
{
	%a = new Camera()
	{
		datablock = Observer;
	...
	%a.mountImage(BNE_UziMagImage,0);
...
datablock ShapeBaseImageData(BNE_UziMagImage)
{
...
function BNE_UziMagImage::onDone(%this,%obj,%slot)
{
...
datablock ShapeBaseImageData(BNE_UziSafetyImage)
{
   shapeFile = "./Uzi/Uzi.dts";
   emap = true;
   mountPoint = 0;
   ...
   hideHands = false;
   scopingImage = BNE_UziIronsightImage;
   safetyImage = BNE_UziImage;
   doColorShift = true;
   colorShiftColor = BNE_UziItem.colorShiftColor;
...
function BNE_UziSafetyImage::onDone(%this,%obj,%slot)
{
	%obj.mountImage(%this.safetyImage, 0);
}

function BNE_UziSafetyImage::onMount(%this,%obj,%slot)
{
	%this.AEMountSetup(%obj, %slot);
...
function BNE_UziSafetyImage::onUnMount(%this, %obj, %slot)
{
	%this.AEUnmountCleanup(%obj, %slot);
...
datablock ShapeBaseImageData(BNE_UziIronsightImage : BNE_UziImage)
{
	recoilHeight = 0.25;

	scopingImage = BNE_UziImage;
	sourceImage = BNE_UziImage;
	
   offset = "0 0 0";
	eyeOffset = "0.00075 1.0 -0.585";
...
function BNE_UziIronsightImage::onDone(%this,%obj,%slot)
{
	%obj.reloadTime[%this.sourceImage.getID()] = getSimTime();
	%obj.mountImage(%this.sourceImage, 0);
}

function BNE_UziIronsightImage::onReady(%this,%obj,%slot)
{
	%obj.baadDisplayAmmo(%this);
}

function BNE_UziIronsightImage::AEOnFire(%this,%obj,%slot)
{	
	%obj.stopAudio(0); 
  %obj.playAudio(0, BNE_UziFire @ getRandom(1, 3) @ Sound);
...
function BNE_UziIronsightImage::onDryFire(%this, %obj, %slot)
{
	%obj.aeplayThread(2, plant);
	serverPlay3D(AEDryFireSound, %obj.getHackPosition());
}
...
function BNE_UziIronsightImage::onMount(%this,%obj,%slot)
{
	%obj.aeplayThread(2, plant);
...
function BNE_UziIronsightImage::onUnMount(%this,%obj,%slot)
{
	if(isObject(%obj.client) && %obj.client.IsA("GameConnection"))
