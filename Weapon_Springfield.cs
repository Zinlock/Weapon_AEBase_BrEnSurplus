...
// item //
//////////
datablock ItemData(BNE_SpringfieldItem)
{
	category = "Weapon";  // Mission editor category
	className = "Weapon"; // For inventory system

	 // Basic Item Properties
	shapeFile = "./Springfield/Springfield.dts";
	rotate = false;
	mass = 1;
	density = 0.2;
	elasticity = 0.2;
	...
	//gui stuff
	uiName = "B&ES: M1903 Springfield";
	iconName = "./Icons/Icon_Springfield";
	doColorShift = true;
	colorShiftColor = "0.75 0.75 0.75 1";

	 // Dynamic properties defined by the scripts
	image = BNE_SpringfieldImage;
	canDrop = true;
...
//weapon image//
////////////////
datablock ShapeBaseImageData(BNE_SpringfieldImage)
{
   // Basic Item properties
   shapeFile = "./Springfield/Springfield.dts";
   emap = true;
   ...
   armReady = true;
   scopingImage = BNE_SpringfieldIronsightImage;
   safetyImage = BNE_SpringfieldSafetyImage;
   doColorShift = true;
   colorShiftColor = BNE_SpringfieldItem.colorShiftColor;
...
function BNE_SpringfieldImage::AEOnFire(%this,%obj,%slot)
{
...
function BNE_SpringfieldImage::onReloadEnd(%this,%obj,%slot)
{
	cancel(%obj.insertshellSchedule);
}

function BNE_SpringfieldImage::onReloadStart(%this, %obj, %slot)
{
	%obj.aeplayThread(2, plant); 
}

function BNE_SpringfieldImage::onDryFire(%this, %obj, %slot)
{
	%obj.aeplayThread(2, plant);
	serverPlay3D(AEDryFireSound, %obj.getHackPosition());
}

function BNE_SpringfieldImage::onReady(%this,%obj,%slot)
{
...
function BNE_SpringfieldImage::onMount(%this,%obj,%slot)
{
	%this.AEMountSetup(%obj, %slot);
	parent::onMount(%this,%obj,%slot);
}

function BNE_SpringfieldImage::onUnMount(%this, %obj, %slot)
{	
...
function BNE_SpringfieldImage::LoadEffect(%this,%obj,%slot)
{
	%obj.reloadSoundSchedule = schedule(175, 0, serverPlay3D, "BNE_MosinInsert" @ getRandom(1, 3) @ "Sound", %obj.getPosition());
    %obj.schedule(100, "aeplayThread", "3", "plant");
    %obj.insertshellSchedule = %this.schedule(200,AEShotgunLoadOne,%obj,%slot);
}

function BNE_SpringfieldImage::AEShotgunLoadOneEffectless(%this,%obj,%slot)
{
	Parent::AEShotgunLoadOne(%this, %obj, %slot);
}

function BNE_SpringfieldImage::onBolt(%this,%obj,%slot)
{
	%obj.aeplayThread(2, plant); 
...
datablock ShapeBaseImageData(BNE_SpringfieldSafetyImage)
{
   shapeFile = "./Springfield/Springfield.dts";
   emap = true;
   mountPoint = 0;
   ...
   hideHands = false;
   scopingImage = BNE_SpringfieldIronsightImage;
   safetyImage = BNE_SpringfieldImage;
   doColorShift = true;
   colorShiftColor = BNE_SpringfieldItem.colorShiftColor;
...
function BNE_SpringfieldSafetyImage::onDone(%this,%obj,%slot)
{
	%obj.mountImage(%this.safetyImage, 0);
}

function BNE_SpringfieldSafetyImage::onMount(%this,%obj,%slot)
{
	%this.AEMountSetup(%obj, %slot);
...
function BNE_SpringfieldSafetyImage::onUnMount(%this, %obj, %slot)
{
	%this.AEUnmountCleanup(%obj, %slot);
...
datablock ShapeBaseImageData(BNE_SpringfieldIronsightImage : BNE_SpringfieldImage)
{
	recoilHeight = 0.5;
...
function BNE_SpringfieldIronsightImage::onDone(%this,%obj,%slot)
{
	%obj.reloadTime[%this.sourceImage.getID()] = getSimTime();
	%obj.mountImage(%this.sourceImage, 0);
}

function BNE_SpringfieldIronsightImage::onReady(%this,%obj,%slot)
{
	%obj.baadDisplayAmmo(%this);
}

function BNE_SpringfieldIronsightImage::AEOnFire(%this,%obj,%slot)
{
	%obj.blockImageDismount = true;
...
function BNE_SpringfieldIronsightImage::onBolt(%this,%obj,%slot)
{
	%obj.aeplayThread(2, plant); 
	schedule(500, 0, serverPlay3D, AEShellRifle @ getRandom(1,2) @ Sound, %obj.getPosition());
}

function BNE_SpringfieldIronsightImage::onDryFire(%this, %obj, %slot)
{
	%obj.aeplayThread(2, plant);
	serverPlay3D(AEDryFireSound, %obj.getHackPosition());
}
...
function BNE_SpringfieldIronsightImage::onMount(%this,%obj,%slot)
{
	%obj.aeplayThread(2, plant);
...
function BNE_SpringfieldIronsightImage::onUnMount(%this,%obj,%slot)
{
	if(isObject(%obj.client) && %obj.client.IsA("GameConnection"))
