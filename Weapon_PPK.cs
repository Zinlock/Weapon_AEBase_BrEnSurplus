datablock AudioProfile(BNE_PPKFire1Sound)
{
   filename    = "./Sounds/Fire/PPK/PPK_FIRE1.wav";
   description = LightClose3D;
   preload = true;
};

datablock AudioProfile(BNE_PPKFire2Sound)
{
   filename    = "./Sounds/Fire/PPK/PPK_FIRE2.wav";
   description = LightClose3D;
   preload = true;
};

datablock AudioProfile(BNE_PPKFire3Sound)
{
   filename    = "./Sounds/Fire/PPK/PPK_FIRE3.wav";
   description = LightClose3D;
   preload = true;
};

// PPK
datablock DebrisData(BNE_PPKMagDebris)
{
	shapeFile = "./PPK/PPKMag.dts";
	lifetime = 2.0;
	minSpinSpeed = -200.0;
	maxSpinSpeed = -100.0;
	elasticity = 0.5;
...
// item //
//////////
datablock ItemData(BNE_PPKItem)
{
	category = "Weapon";  // Mission editor category
	className = "Weapon"; // For inventory system

	 // Basic Item Properties
	shapeFile = "./PPK/PPK.dts";
	rotate = false;
	mass = 1;
	density = 0.2;
	elasticity = 0.2;
...
//weapon image//
////////////////
datablock ShapeBaseImageData(BNE_PPKImage)
{
   // Basic Item properties
   shapeFile = "./PPK/PPK.dts";
   emap = true;
   ...
   //raise your arm up or not
	armReady = true;
	hideHands = false;
	safetyImage = BNE_PPKSafetyImage;
    scopingImage = BNE_PPKIronsightImage;
	doColorShift = true;
	colorShiftColor = BNE_PPKItem.colorShiftColor;//"0.400 0.196 0 1.000";
...
function BNE_PPKImage::AEOnFire(%this,%obj,%slot)
{	
	%obj.stopAudio(0); 
  %obj.playAudio(0, BNE_PPKFire @ getRandom(1, 3) @ Sound);
...
function BNE_PPKImage::onDryFire(%this, %obj, %slot)
{
	%obj.aeplayThread(2, plant);
	serverPlay3D(AEDryFireSound, %obj.getHackPosition());
}

function BNE_PPKImage::onReloadMagIn(%this,%obj,%slot)
{
...
function BNE_PPKImage::onReload2MagIn(%this,%obj,%slot)
{
   %obj.schedule(50, "aeplayThread", "2", "shiftright");
   %obj.schedule(50, "aeplayThread", "3", "plant");
   %obj.schedule(300, "aeplayThread", "3", "shiftleft");
...
function BNE_PPKImage::onReloadEnd(%this,%obj,%slot)
{
	Parent::AEMagReloadAll(%this, %obj, %slot);
}

function BNE_PPKImage::onReload2End(%this,%obj,%slot)
{
	Parent::AEMagReloadAll(%this, %obj, %slot);
}
...
function BNE_PPKImage::onReloadStart(%this,%obj,%slot)
{
   %obj.schedule(300, "aeplayThread", "3", "shiftleft");
...
function BNE_PPKImage::onReady(%this,%obj,%slot)
{
...
function BNE_PPKImage::onMount(%this,%obj,%slot)
{
	%obj.aeplayThread(2, plant);
...
function BNE_PPKImage::onUnMount(%this,%obj,%slot)
{
...
function BNE_PPKImage::onMagDrop(%this,%obj,%slot)
{
	%a = new Camera()
	{
		datablock = Observer;
...
datablock ShapeBaseImageData(BNE_PPKMagImage)
{
...
function BNE_PPKMagImage::onDone(%this,%obj,%slot)
{
...
datablock ShapeBaseImageData(BNE_PPKSafetyImage)
{
   shapeFile = "./PPK/PPK.dts";
   emap = true;
   mountPoint = 0;
   ...
   hideHands = false;
   scopingImage = BNE_PPKIronsightImage;
   safetyImage = BNE_PPKImage;
   doColorShift = true;
   colorShiftColor = BNE_PPKItem.colorShiftColor;
...
function BNE_PPKSafetyImage::onDone(%this,%obj,%slot)
{
	%obj.mountImage(%this.safetyImage, 0);
}

function BNE_PPKSafetyImage::onMount(%this,%obj,%slot)
{
	%this.AEMountSetup(%obj, %slot);
...
function BNE_PPKSafetyImage::onUnMount(%this, %obj, %slot)
{
	%this.AEUnmountCleanup(%obj, %slot);
...
datablock ShapeBaseImageData(BNE_PPKIronsightImage : BNE_PPKImage)
{
	recoilHeight = 0.11;

	scopingImage = BNE_PPKImage;
	sourceImage = BNE_PPKImage;
...
function BNE_PPKIronsightImage::onDone(%this,%obj,%slot)
{
	%obj.reloadTime[%this.sourceImage.getID()] = getSimTime();
	%obj.mountImage(%this.sourceImage, 0);
}

function BNE_PPKIronsightImage::onReady(%this,%obj,%slot)
{
	%obj.baadDisplayAmmo(%this);
}

function BNE_PPKIronsightImage::AEOnFire(%this,%obj,%slot)
{	
	%obj.stopAudio(0); 
  %obj.playAudio(0, BNE_PPKFire @ getRandom(1, 3) @ Sound);
...
function BNE_PPKIronsightImage::onDryFire(%this, %obj, %slot)
{
	%obj.aeplayThread(2, plant);
	serverPlay3D(AEDryFireSound, %obj.getHackPosition());
}
...
function BNE_PPKIronsightImage::onMount(%this,%obj,%slot)
{
	%obj.aeplayThread(2, plant);
...
function BNE_PPKIronsightImage::onUnMount(%this,%obj,%slot)
{
	if(isObject(%obj.client) && %obj.client.IsA("GameConnection"))
