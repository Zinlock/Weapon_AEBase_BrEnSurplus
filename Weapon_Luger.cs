datablock AudioProfile(BNE_LugerFire1Sound)
{
   filename    = "./Sounds/Fire/Luger/Luger_FIRE_1.wav";
   description = LightClose3D;
   preload = true;
};

datablock AudioProfile(BNE_LugerFire2Sound)
{
   filename    = "./Sounds/Fire/Luger/Luger_FIRE_2.wav";
   description = LightClose3D;
   preload = true;
};

datablock AudioProfile(BNE_LugerFire3Sound)
{
   filename    = "./Sounds/Fire/Luger/Luger_FIRE_3.wav";
   description = LightClose3D;
   preload = true;
};

// Luger
datablock DebrisData(BNE_LugerMagDebris)
{
	shapeFile = "./Luger/LugerMag.dts";
	lifetime = 2.0;
	minSpinSpeed = -200.0;
	maxSpinSpeed = -100.0;
	elasticity = 0.5;
...
// item //
//////////
datablock ItemData(BNE_LugerItem)
{
	category = "Weapon";  // Mission editor category
	className = "Weapon"; // For inventory system

	 // Basic Item Properties
	shapeFile = "./Luger/Luger.dts";
	rotate = false;
	mass = 1;
	density = 0.2;
	elasticity = 0.2;
	...
	recoil = "Light"; 
	uiColor = "1 1 1";
	description = "The Luger is a German pistol first patented by Georg Luger aiming to improve on the Borchardt C-93 pistol.";
...
//weapon image//
////////////////
datablock ShapeBaseImageData(BNE_LugerImage)
{
   // Basic Item properties
   shapeFile = "./Luger/Luger.dts";
   emap = true;
   ...
   //raise your arm up or not
	armReady = true;
	hideHands = false;
	safetyImage = BNE_LugerSafetyImage;
    scopingImage = BNE_LugerIronsightImage;
	doColorShift = true;
	colorShiftColor = BNE_LugerItem.colorShiftColor;//"0.400 0.196 0 1.000";
...
function BNE_LugerImage::AEOnFire(%this,%obj,%slot)
{	
	%obj.stopAudio(0); 
  %obj.playAudio(0, BNE_LugerFire @ getRandom(1, 3) @ Sound);
...
function BNE_LugerImage::onDryFire(%this, %obj, %slot)
{
	%obj.aeplayThread(2, plant);
	serverPlay3D(AEDryFireSound, %obj.getHackPosition());
}

function BNE_LugerImage::onReloadMagIn(%this,%obj,%slot)
{
...
function BNE_LugerImage::onReload2MagIn(%this,%obj,%slot)
{
   %obj.schedule(50, "aeplayThread", "2", "shiftright");
   %obj.schedule(50, "aeplayThread", "3", "plant");
...
function BNE_LugerImage::onReloadEnd(%this,%obj,%slot)
{
	Parent::AEMagReloadAll(%this, %obj, %slot);
}

function BNE_LugerImage::onReload2End(%this,%obj,%slot)
{
	Parent::AEMagReloadAll(%this, %obj, %slot);
}
...
function BNE_LugerImage::onReloadStart(%this,%obj,%slot)
{
...
function BNE_LugerImage::onReady(%this,%obj,%slot)
{
...
function BNE_LugerImage::onMount(%this,%obj,%slot)
{
	%obj.aeplayThread(2, plant);
...
function BNE_LugerImage::onUnMount(%this,%obj,%slot)
{
...
function BNE_LugerImage::onMagDrop(%this,%obj,%slot)
{
	%a = new Camera()
	{
		datablock = Observer;
	...
	%a.mountImage(BNE_LugerMagImage,0);
...
datablock ShapeBaseImageData(BNE_LugerMagImage)
{
...
function BNE_LugerMagImage::onDone(%this,%obj,%slot)
{
...
datablock ShapeBaseImageData(BNE_LugerSafetyImage)
{
   shapeFile = "./Luger/Luger.dts";
   emap = true;
   mountPoint = 0;
   ...
   hideHands = false;
   scopingImage = BNE_LugerIronsightImage;
   safetyImage = BNE_LugerImage;
   doColorShift = true;
   colorShiftColor = BNE_LugerItem.colorShiftColor;
...
function BNE_LugerSafetyImage::onDone(%this,%obj,%slot)
{
	%obj.mountImage(%this.safetyImage, 0);
}

function BNE_LugerSafetyImage::onMount(%this,%obj,%slot)
{
	%this.AEMountSetup(%obj, %slot);
...
function BNE_LugerSafetyImage::onUnMount(%this, %obj, %slot)
{
	%this.AEUnmountCleanup(%obj, %slot);
...
datablock ShapeBaseImageData(BNE_LugerIronsightImage : BNE_LugerImage)
{
	recoilHeight = 0.2;

	scopingImage = BNE_LugerImage;
	sourceImage = BNE_LugerImage;
	
   offset = "0 0 0.";
	eyeOffset = "0.0036 1.25 -0.55";
...
function BNE_LugerIronsightImage::onDone(%this,%obj,%slot)
{
	%obj.reloadTime[%this.sourceImage.getID()] = getSimTime();
	%obj.mountImage(%this.sourceImage, 0);
}

function BNE_LugerIronsightImage::onReady(%this,%obj,%slot)
{
	%obj.baadDisplayAmmo(%this);
}

function BNE_LugerIronsightImage::AEOnFire(%this,%obj,%slot)
{	
	%obj.stopAudio(0); 
  %obj.playAudio(0, BNE_LugerFire @ getRandom(1, 3) @ Sound);
...
function BNE_LugerIronsightImage::onDryFire(%this, %obj, %slot)
{
	%obj.aeplayThread(2, plant);
	serverPlay3D(AEDryFireSound, %obj.getHackPosition());
}
...
function BNE_LugerIronsightImage::onMount(%this,%obj,%slot)
{
	%obj.aeplayThread(2, plant);
...
function BNE_LugerIronsightImage::onUnMount(%this,%obj,%slot)
{
	if(isObject(%obj.client) && %obj.client.IsA("GameConnection"))
