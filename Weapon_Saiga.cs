// datablock AudioProfile(BNE_SaigaFireLoopSound)
// {
//    filename    = "./Sounds/Fire/Saiga/AK_LP.wav";
//    description = BAADFireMediumLoop3D;
//    preload = true;
// };

// datablock AudioProfile(BNE_SaigaFireLoopEndSound)
// {
//    filename    = "./Sounds/Fire/Saiga/AK_LP_END.wav";
//    description = MediumClose3D;
//    preload = true;
// };

datablock AudioProfile(BNE_SaigaFire1Sound)
{
  filename    = "./Sounds/Fire/Saiga/Saiga_fire1.wav";
  description = MediumClose3D;
  preload = true;
};

datablock AudioProfile(BNE_SaigaFire2Sound)
{
  filename    = "./Sounds/Fire/Saiga/Saiga_fire2.wav";
  description = MediumClose3D;
  preload = true;
};

datablock AudioProfile(BNE_SaigaFire3Sound)
{
  filename    = "./Sounds/Fire/Saiga/Saiga_fire3.wav";
  description = MediumClose3D;
  preload = true;
};
...
// item //
//////////
datablock ItemData(BNE_SaigaItem)
{
	category = "Weapon";  // Mission editor category
	className = "Weapon"; // For inventory system

	 // Basic Item Properties
	shapeFile = "./Saiga/Saiga.dts";
	rotate = false;
	mass = 1;
	density = 0.2;
	elasticity = 0.2;
	...
	//gui stuff
	uiName = "B&ES: Saiga 12";
	iconName = "./Icons/53";
	doColorShift = true;
	colorShiftColor = "0.4 0.4 0.4 1";

	 // Dynamic properties defined by the scripts
	image = BNE_SaigaImage;
	canDrop = true;
...
//weapon image//
////////////////
datablock ShapeBaseImageData(BNE_SaigaImage)
{
   // Basic Item properties
   shapeFile = "./Saiga/Saiga.dts";
   emap = true;
   ...
   //raise your arm up or not
	armReady = true;
	hideHands = false;
	safetyImage = BNE_SaigaSafetyImage;
    scopingImage = BNE_SaigaIronsightImage;
	doColorShift = true;
	colorShiftColor = BNE_SaigaItem.colorShiftColor;//"0.400 0.196 0 1.000";
  ...
  //loopingEndSound = BNE_SaigaFireLoopEndSound;
...
function BNE_SaigaImage::AEOnFire(%this,%obj,%slot)
{
	// %obj.playAudio(0, BNE_SaigaFireLoopSound);
  //   %obj.FireLoop = true;

	%obj.stopAudio(0); 
  %obj.playAudio(0, BNE_SaigaFire @ getRandom(1, 3) @ Sound);
...
function BNE_SaigaImage::onEndLoop(%this, %obj, %slot)
{
	// %obj.playAudio(0, %this.loopingEndSound);
	// %obj.FireLoop = false;
}

function BNE_SaigaImage::onDryFire(%this, %obj, %slot)
{
	%obj.aeplayThread(2, plant);
	serverPlay3D(AEDryFireSound, %obj.getHackPosition());
}

function BNE_SaigaImage::onReloadMagIn(%this,%obj,%slot)
{
   %obj.schedule(50, "aeplayThread", "2", "plant");
}

function BNE_SaigaImage::onReload2MagIn(%this,%obj,%slot)
{
   %obj.schedule(50, "aeplayThread", "2", "plant");
}

function BNE_SaigaImage::onReloadMagOut(%this,%obj,%slot)
{
   %obj.aeplayThread(2, plant);
}

function BNE_SaigaImage::onReload2MagOut(%this,%obj,%slot)
{
   %obj.aeplayThread(2, plant);
}
...
function BNE_SaigaImage::onReloadEnd(%this,%obj,%slot)
{
	Parent::AEMagReloadAll(%this, %obj, %slot);
}
...
function BNE_SaigaImage::onReloadStart(%this,%obj,%slot)
{
   %obj.aeplayThread(2, plant);
...
function BNE_SaigaImage::onReady(%this,%obj,%slot)
{
...
function BNE_SaigaImage::onMount(%this,%obj,%slot)
{
	%obj.aeplayThread(2, plant);
...
function BNE_SaigaImage::onUnMount(%this,%obj,%slot)
{
...
function BNE_SaigaImage::onMagDrop(%this,%obj,%slot)
{
	%a = new Camera()
	{
		datablock = Observer;
...
datablock ShapeBaseImageData(BNE_SaigaSafetyImage)
{
   shapeFile = "./Saiga/Saiga.dts";
   emap = true;
   mountPoint = 0;
   ...
   hideHands = false;
   scopingImage = BNE_SaigaIronsightImage;
   safetyImage = BNE_SaigaImage;
   doColorShift = true;
   colorShiftColor = BNE_SaigaItem.colorShiftColor;
...
function BNE_SaigaSafetyImage::onDone(%this,%obj,%slot)
{
	%obj.mountImage(%this.safetyImage, 0);
}

function BNE_SaigaSafetyImage::onMount(%this,%obj,%slot)
{
	%this.AEMountSetup(%obj, %slot);
...
function BNE_SaigaSafetyImage::onUnMount(%this, %obj, %slot)
{
	%this.AEUnmountCleanup(%obj, %slot);
...
function BNE_SaigaIronsightImage::onDone(%this,%obj,%slot)
{
	%obj.reloadTime[%this.sourceImage.getID()] = getSimTime();
	%obj.mountImage(%this.sourceImage, 0);
}

function BNE_SaigaIronsightImage::onReady(%this,%obj,%slot)
{
	%obj.baadDisplayAmmo(%this);
}

function BNE_SaigaIronsightImage::AEOnFire(%this,%obj,%slot)
{
	%obj.playAudio(0, BNE_SaigaFireLoopSound);
	// %obj.playAudio(0, BNE_SaigaFireLoopSound);
  //   %obj.FireLoop = true;
...
function BNE_SaigaIronsightImage::onEndLoop(%this, %obj, %slot)
{
	// %obj.playAudio(0, %this.loopingEndSound);
	// %obj.FireLoop = false;
}

function BNE_SaigaIronsightImage::onDryFire(%this, %obj, %slot)
{
	%obj.aeplayThread(2, plant);
	serverPlay3D(AEDryFireSound, %obj.getHackPosition());
}
...
function BNE_SaigaIronsightImage::onMount(%this,%obj,%slot)
{
	%obj.aeplayThread(2, plant);
...
function BNE_SaigaIronsightImage::onUnMount(%this,%obj,%slot)
{
	if(isObject(%obj.client) && %obj.client.IsA("GameConnection"))
